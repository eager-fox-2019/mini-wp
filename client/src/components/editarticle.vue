        ...
        <div class="col-12 my-2">
          <div class="row mt-2">
            <div class="col-auto d-flex">
              <label for="selectedPicture">Change Featured Image</label>
...
export default {
  ...
  methods: {
    revertEdit() {
      ...
      quill.root.innerHTML = this.selectedArticle.rawHTML;
    ...
    editArticle(status) {
      ...
      if (
        status.toLowerCase() === "post" &&
        this.selectedArticle.status == "save"
      ) {
        updVal.postedAt = new Date();
      }

      let errmsg = "";
      if (updVal.rawHTML == "<p><br></p>" || updVal.content == "\n") {
      ...
      if (errmsg !== "") {
        ...
        swal({
          title: "Confirmation",
          text: `Update this article?`,
          icon: "info",
          buttons: true,
          dangerMode: true
        }).then(confirm => {
          if (confirm && typeof updVal.picture !== "string") {
            const blob = new Blob([updVal.picture], {
              type: updVal.picture.type
            });
            ...
            this.ax({
              ...
              .then(({ data }) => {
                $("#selectedPicture").val("");
                this.autosave();
                swal("Yay", `Article updated`, "success");
              })
              .catch(err => {
          ...
          } else if (confirm && typeof updVal.picture == "string") {
            this.ax({
              method: "PATCH",
              url: "/articles/" + this.selectedArticle._id,
              data: updVal
